...
public partial class HybridCache
{
    public async Task HashSetAsync(string key, IDictionary<string, string> fields, TimeSpan? redisExpiry = null,
        Condition when = Condition.Always, Flags flags = Flags.PreferMaster, CancellationToken token = default)
    {
        ...
        await RedisDb.HashFieldSetAndSetExpiryAsync(cacheKey, hashSet, redisExpiry, false, (When)when, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }

    public async Task<bool> HashSetAsync(string key, string hashField, string value, Condition when = Condition.Always,
        Flags flags = Flags.PreferMaster, CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashSetAsync(cacheKey, hashField, value, (When)when, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        var fields = await RedisDb.HashGetAllAsync(cacheKey, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashGetAsync(cacheKey, hashField, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        var values = await RedisDb.HashGetAsync(cacheKey, hashFields.Select(k => (RedisValue)k).ToArray(), (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashExistsAsync(cacheKey, hashField, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashDeleteAsync(cacheKey, hashField, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashDeleteAsync(cacheKey, hashFields.Select(k => (RedisValue)k).ToArray(), (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }

    public async IAsyncEnumerable<KeyValuePair<string, string>> HashScanAsync(string key, string pattern,
        Flags flags = Flags.None, [EnumeratorCancellation] CancellationToken token = default)
    {
        var cacheKey = GetCacheKey(key);
    ...
    public async IAsyncEnumerable<string> HashScanNoValuesAsync(string key, string pattern,
        Flags flags = Flags.None, [EnumeratorCancellation] CancellationToken token = default)
    {
        var cacheKey = GetCacheKey(key);
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        var values = await RedisDb.HashValuesAsync(cacheKey, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
        return values.Select(v => v.ToString()).ToArray();
    }
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        var keys = await RedisDb.HashKeysAsync(cacheKey, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        var value = await RedisDb.HashFieldGetAndDeleteAsync(cacheKey, hashField, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
        ...
        CancellationToken token = default)
    {
        token.ThrowIfCancellationRequested();
        var cacheKey = GetCacheKey(key);
        return await RedisDb.HashLengthAsync(cacheKey, (CommandFlags)flags)
            .Cancelable(token).ConfigureAwait(false);
    }
}
