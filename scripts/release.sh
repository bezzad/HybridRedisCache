#!/usr/bin/env bash
# HybridRedisCache — one-shot NuGet release routine.
#
...
#   scripts/release.sh 6.0.1                # non-interactive version
#   scripts/release.sh 6.0.1 --yes          # also skip the confirmation prompt
#   scripts/release.sh 6.0.1 --notes-file notes.md
#   scripts/release.sh 6.0.1 --skip-wait    # push the tag and exit; don't wait on CI / set notes
#
...
REPO="bezzad/HybridRedisCache"
MAIN_BRANCH="main"
DEV_BRANCH="develop"
CSPROJ="src/HybridRedisCache/HybridRedisCache.csproj"
...
echo "    publish : nuget.org + GitHub Release (via .github/workflows/$WORKFLOW_FILE, triggered by the tag push)"
...
step "Tagging $TAG"
git tag -a "$TAG" -m "HybridRedisCache $TAG"
git push origin "$TAG"
ok "pushed $TAG — release.yml is now packing + publishing to nuget.org"
...
echo "    nuget.org        : https://www.nuget.org/packages/HybridRedisCache/$VERSION"
echo
